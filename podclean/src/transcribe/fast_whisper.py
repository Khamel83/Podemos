...
def get_whisper_model(model_size: str = "small", device: str = "auto", compute_type: str = "auto"):
    global _model
    if _model is None:
...
def fast_transcribe(audio_path: str, model_size: str = "small", vad: bool = True, beam_size: int = 1, word_timestamps: bool = True):
    """
    Performs fast transcription of an audio file using faster-whisper.

    Args:
        audio_path: Path to the audio file.
        model_size: Size of the Whisper model to use (e.g., "small", "medium").
    ...
    model = get_whisper_model(model_size, device="auto", compute_type="auto") # Use auto for device and compute_type
    ...
    print(f"Transcription complete for {audio_path}. Language: {info.language}, Duration: {info.duration_after_vad}s")
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_audio_path):
        ...
        results = fast_transcribe(test_audio_path, model_size="tiny", vad=True, beam_size=1, word_timestamps=True)
